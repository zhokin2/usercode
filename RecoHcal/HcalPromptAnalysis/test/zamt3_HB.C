...
const Int_t FlagOrdinateValue= 1;// =0 absolut_values; =1 normilized on 1st LED run;  0 1 1 1
...
const Int_t NMethodsStart =37;     // since which method to start ( min=  1)
...
// //    if(Method > 5 && Method < 21 ) NR = 1;// only averaged values, one dependence only
    ...
    //---=[ SetCanvasDef           ]=----------------
    // Create a new canvas.
    c1 = new TCanvas("zamt3_HB","Monte Carlo Study of Z scaling",10,40,800,600);
    c1->Range(0,0,25,18);
    c1->SetFillColor(40);
    c1->Clear();
    //      c1->Divide(2,2);
	...
	if(FlagWhatToImpose >= 1 ) {pad1->DrawFrame(1,   0.0,  maxdays,   5.0);}
	else {
