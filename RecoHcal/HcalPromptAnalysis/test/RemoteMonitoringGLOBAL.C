...
//root -b -q -l 'RemoteMonitoringGLOBAL.C+("/afs/cern.ch/work/z/zhokin/hcal/CMSSW_5_3_21/src/RecoHcal/HcalPromptAnalysis/test/Global_227391.root")'
...
void RemoteMonitoringGLOBAL(const char* fname = "/afs/cern.ch/work/z/zhokin/hcal/CMSSW_5_3_21/src/RecoHcal/HcalPromptAnalysis/test/Global_227391.root")
{
    gROOT->Reset();
      ...
      for (int sub=1;sub<=4;sub++) {  //Subdetector: 1-HB, 2-HE, 3-HF, 4-HO
	   ...
	   htmlFile << "   td.s5 { font-family: arial, arial ce, helvetica; background-color: #00FF00; }"<< std::endl;
	      ...
	      if (test !=0) htmlFile << "<h2> 3. Distribution of estimator averaged over events in LS and over all channels for each LS </h2>"<< std::endl;
	      	      if (test ==0) {
	          if (sub==1) htmlFile << "<h2> 3.  Portion of events with Nbcs>"<<CutPo[sub][1]<<" (Depth1), "<<CutPo[sub][2] <<" (Depth2) in each event.</h2>"<< std::endl;
                  if (sub==2) htmlFile << "<h2> 3.  Portion of events with Nbcs>"<<CutPo[sub][1]<<" (Depth1), "<<CutPo[sub][2] <<" (Depth2), "<<CutPo[sub][3] <<" (Depth3) in each event.</h2>"<< std::endl;
                  if (sub==3) htmlFile << "<h2> 3.  Portion of events with Nbcs>"<<CutPo[sub][1]<<" (Depth4) in each event.</h2>"<< std::endl;
                  if (sub==4) htmlFile << "<h2> 3.  Portion of events with Nbcs>"<<CutPo[sub][1]<<" (Depth1), "<<CutPo[sub][2] <<" (Depth2) in each event.</h2>"<< std::endl;
