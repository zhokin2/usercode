...
//root -b -q -l 'RemoteMonitoringGLOBAL.C+("/afs/cern.ch/work/d/dtlisov/private/Monitoring/histos/Global_198487.root")'
...
void RemoteMonitoringGLOBAL(const char* fname = "/afs/cern.ch/work/d/dtlisov/private/Monitoring/histos/Global_198487.root")
{
    gROOT->Reset();
...
//    TFile *hfile= new TFile("GlobalHist.root", "READ");
    ...
    // Cut [test][sub][depth]
    double Cut0[7][5][5]={{{0.,0.,0.,0.,0.},{0.,3.1,0.1,0.,0.},{0.,1.5,1.5,0.4,0.},{0.,0.,0.,0.,100.},{0.,0.1,0.1,0.,0.}},  //CapID
                          {{0.,0.,0.,0.,0.},{0.,100.,35.,0.,0.},{0.,40.,35.,40.,0.},{0.,0.,0.,0.,2100.},{0.,120.,60.,0.,0.}},  //Amplitude   
    ...
    double CutAb[5][5]={{0.,0.,0.,0.,0.},{0.,20.,7.,0.,0.},{0.,16.,13.,4.,0.},{0.,0.,0.,0.,45.},{0.,10.,5.,0.,0.}};
    double CutPo[5][5]={{0.,0.,0.,0.,0.},{0.,19.,6.,0.,0.},{0.,15.,12.,3.,0.},{0.,0.,0.,0.,40.},{0.,9.,4.,0.,0.}};
//======================================================================
...
//+++++++++++++++++++++++++++++ 
      ...
      LumLum->SetTitle("LS number per iLS  \b");
      LumLum->SetXTitle("LS number \b");
      LumLum->SetYTitle("iLs \b");
      ...
      cHB->cd(2);
      TH1F *LumiEv= (TH1F*)hfile->Get("h_nevents_per_eachLS");
      gPad->SetGridy();
      gPad->SetGridx();
      gPad->SetLogy();
      //            gPad->SetLogx();
      LumiEv->GetYaxis()->SetLabelSize(0.04);
      LumiEv->SetTitle("Number of events per iLS");
      LumiEv->SetXTitle("iLS");
...
//+++++++++++++++++++++++++++++   
    ...
    for (int test=0;test<=5;test++) { //Test: 0, 
        for (int sub=1;sub<=4;sub++) {  //Subdetector: 1-HB, 2-HE, 3-HF, 4-HO
	        ...
	        if (sub==3) cONE->cd(k-3);
	        if (sub==4) cHB->cd(k);      
	        if (MapNumChanDepth[test][sub][k]->IsA()->InheritsFrom("TH2F"))        
		    MapNumBadChanDepth[test][sub][k]->Divide(MapNumBadChanDepth[test][sub][k],MapNumChanDepth[test][sub][k], 1, 1, "B");
		if (!((k==1)&&(sub==1))) {
		   for (int bin=1; bin <= MapNumBadChanDepth[test][sub][k]-> GetNbinsX(); bin++  ) {
		        MapNumBadChanFull[test]->Add(MapNumBadChanDepth[test][sub][k]);
//		        MapNumChanFull[test]->Add(MapNumChanDepth[test][sub][k]); 
	           }
		}
...
/*
		if (k==1&&sub==1) {
		    HistNumBadChanFull[test]->Divide(HistNumBadChanDepth[test][sub][k],HistNumChanDepth[test][sub][k], 1, 1, "B");
		}
		else {
		   HistNumBadChanFull[test]->Add(HistNumBadChanDepth[test][sub][k]);
		}
*/
		...
		if (test != 0) HistNumBadChanDepth[test][sub][k]->SetYTitle("Averaged estimator \b");
                HistNumBadChanDepth[test][sub][k]->SetMarkerColor(1);
...
//+++++++++++++++++++++++++++++  
	...
	for (int k=k_min[sub];k<=k_max[sub];k++) {  //Depth 
	  ...
	  gPad->SetGridx();
	  //  gPad->SetLogy();      
	  if (HistAbnormNumBadChanDepth[sub][k]->IsA()->InheritsFrom("TH1F"))
...
//+++++++++++++++++++++++++++++  
	...
	for (int k=k_min[sub];k<=k_max[sub];k++) {  //Depth 
	  ...
	  gPad->SetGridx();
	  //  gPad->SetLogy();      
 	  if (HistPortBadEventsDepth[sub][k]->IsA()->InheritsFrom("TH1F"))
  ...
  for (int test=0;test<=5;test++) { //Test: 0,   
      for (int sub=1;sub<=4;sub++) {  //Subdetector: 1-HB, 2-HE, 3-HF, 4-HO
	   ...
	   htmlFile << "   td.s5 { font-family: arial, arial ce, helvetica; background-color: #00FF00; }"<< std::endl;
	      ...
	      htmlFile << "<h3> Legend: Overflow corresponds to BAD LS candidates.</h3>"<< std::endl; 
	      ...
	      if (test ==0) htmlFile << "<h2> 2. Averaged number of bad channels for each LS </h2>"<< std::endl;
              if (test !=0) htmlFile << "<h3> Channel legend: white - good, other colour - bad.  </h3>"<< std::endl;
	      if (test ==0) htmlFile << "<h3> Legends: dots above line correspond BAD LS  </h3>"<< std::endl;
              if (test==0){
	      ...
	      if (test !=0) htmlFile << "<h2> 3. Distribution of estimator averaged over events in LS and over all channels for each LS </h2>"<< std::endl;
	      	      if (test ==0) {
	          if (sub==1) htmlFile << "<h2> 1.  Portion of events with Nbcs>"<<CutPo[sub][1]<<" (Depth1), "<<CutPo[sub][2] <<" (Depth2) in each event.</h2>"<< std::endl;
                  if (sub==2) htmlFile << "<h2> 1.  Portion of events with Nbcs>"<<CutPo[sub][1]<<" (Depth1), "<<CutPo[sub][2] <<" (Depth2), "<<CutPo[sub][3] <<" (Depth3) in each event.</h2>"<< std::endl;
                  if (sub==3) htmlFile << "<h2> 1.  Portion of events with Nbcs>"<<CutPo[sub][1]<<" (Depth4) in each event.</h2>"<< std::endl;
                  if (sub==4) htmlFile << "<h2> 1.  Portion of events with Nbcs>"<<CutPo[sub][1]<<" (Depth1), "<<CutPo[sub][2] <<" (Depth2) in each event.</h2>"<< std::endl;
		 ...
		 htmlFile << "<h3> Legends: Red boxes correspond BAD LS selected with following cuts: "<<Cut0[test][sub][4]<<" (Depth4)</h3>"<< std::endl;
	      ...
	      for (int k=k_min[sub];k<=k_max[sub]; k++) htmlFile << "<td class=\"s1\" align=\"center\">< Nbcs > Depth "<< k <<" </td>"  << std::endl;
...
// Creating tests  html pages:
 ...
 for (int test=0;test<=5;test++) { //Test: 0,   
     ...
     htmlFile << "<br>"<< std::endl;
     htmlFile << "<h2> 2.  Average value for whole HCAL </h2>"<< std::endl;   
     htmlFile << "<br>"<< std::endl;      
...
// Creating description html file: 
     ...
     htmlFile << "<br>"<< std::endl;
     htmlFile << "<h3> - c means calibration channels. For example Am means Amplitude criteria for calibration channels </h3> "<< std::endl;
     htmlFile << "<br>"<< std::endl;
     htmlFile << "</body> " << std::endl;
     htmlFile << "</html> " << std::endl; 
     htmlFile.close();
