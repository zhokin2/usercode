...
// //    if(Method > 5 && Method < 21 ) NR = 1;// only averaged values, one dependence only
    ...
    //---=[ SetCanvasDef           ]=----------------
    // Create a new canvas.
    c1 = new TCanvas("zamt3_HE","Monte Carlo Study of Z scaling",10,40,800,600);
    c1->Range(0,0,25,18);
    c1->SetFillColor(40);
    c1->Clear();
    //      c1->Divide(2,2);
	...
	pad1->DrawFrame(1,   0.99,  maxdays,   1.01);// all 
      ...
      if(Method ==36&&YEAR==0) pad1->DrawFrame(1,    0.,  maxdays,     0.01);  //36-CapIdErrors 
      ...
      if(Method ==37&&YEAR==0) pad1->DrawFrame(1,    0.,  maxdays,      1.);  //37-NbCapIdErrors 
