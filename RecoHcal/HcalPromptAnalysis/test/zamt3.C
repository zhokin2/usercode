...
const Int_t NMethodsStart =26;     // since which method to start ( min=  1)
...
void zamt3() {
  ...
  while (Method<NMethodsToUse+1) {
    ...
    //---=[ SetCanvasDef           ]=----------------
    // Create a new canvas.
    c1 = new TCanvas("zamt3","Monte Carlo Study of Z scaling",10,40,800,600);
    c1->Range(0,0,25,18);
    c1->SetFillColor(40);
    c1->Clear();
    //      c1->Divide(2,2);
	...
	//
	  ...
	  //
	  //
	  if(Method < 6 || Method == 34) TH2F *twod0= (TH2F*)hfile->Get("h_mapDepth1_HB");
	  ...
	  if(Method != 19 && Method != 20) {
		...
		while (rr==0) {
		  ...
		  if(rrcount>100) {
		    ...
		    cout<<"rr exceed 100, seems empty file = "<<k1<<endl;
		  }
	...
	for(int k2 = 0; k2<NP; k2++) {
	  ZZ[k2] = YY[k2]/YY[1];// normalization on 1st time point
	...
	if(FlagOrdinateValue !=0 ) {
	  for(int k2 = 0; k2<NP; k2++) {
	    ZZ[k2] = YY2[k2]/YY2[1];// normalization on 1st time point
	...
	if(FlagOrdinateValue !=0 ) {
	  for(int k2 = 0; k2<NP; k2++) {
	    ZZ[k2] = YY3[k2]/YY3[1];// normalization on 1st time point
