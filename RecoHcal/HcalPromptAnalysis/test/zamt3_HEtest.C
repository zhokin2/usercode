...
const Int_t NMethodsStart =36;     // since which method to start ( min=  1)
...
// //    if(Method > 5 && Method < 21 ) NR = 1;// only averaged values, one dependence only
    ...
    //---=[ SetCanvasDef           ]=----------------
    // Create a new canvas.
    c1 = new TCanvas("zamt3_HEtest","Monte Carlo Study of Z scaling",10,40,800,600);
    c1->Range(0,0,25,18);
    c1->SetFillColor(40);
    c1->Clear();
    //      c1->Divide(2,2);
    ...
    if(Method == 36) t->DrawLatex(0.9,15.5,"Time dependence of CapId errors averaged over events of each LED run for some random HE channels");
	...
	if(FlagWhatToImpose >= 1 ) {
	  pad1->DrawFrame(1,   0.0,  maxdays,   3.0);
	  if(YEAR == 2 ) pad1->DrawFrame(1,   0.0,  maxdays,   5.0);
	  if(YEAR < 0 ) pad1->DrawFrame(1,   0.0,  maxdays,  10.0);
	}
	else {
	  if(YEAR < 0 ) pad1->DrawFrame(1,   0.0,  maxdays,  10.0);
	...
	pad1->DrawFrame(1,   0.97,  maxdays,   1.03);// all 
      }
      else if(Method== 7 ){
	pad1->DrawFrame(1,   0.98,  maxdays,   1.02);// all 
      }
      else if(Method== 6 ){
	pad1->DrawFrame(1,   0.93,  maxdays,   1.07);// all 
      }
      else if(Method== 9 || Method== 10|| Method== 19 || Method== 21 ){
	pad1->DrawFrame(1,   0.95,  maxdays,   1.05);// all 
      }
      else if(Method== 11 ){
	pad1->DrawFrame(1,   0.97,  maxdays,   1.03);// all 
      }
      else if(Method== 13 || Method== 14  ){
	pad1->DrawFrame(1,   0.95,  maxdays,   1.05);// all 
      }
      else if(Method== 20  ){
	pad1->DrawFrame(1,   0.90,  maxdays,   1.10);// all 
      }
      else if(Method== 22 || Method== 25  ){
	pad1->DrawFrame(1,   0.90,  maxdays,   1.10);// all 
      }
      else if(Method== 23 ){
	pad1->DrawFrame(1,   0.95,  maxdays,   1.05);// all 
	...
	pad1->DrawFrame(1,   0.90,  maxdays,   1.20);// all 
      }
      else if(Method== 29  ){
	pad1->DrawFrame(1,   0.90,  maxdays,   1.10);// all 
      }
      else if(Method== 26 || Method== 27 || Method== 28  ){
	pad1->DrawFrame(1,   0.97,  maxdays,   1.03);// all 
      }
      else {
	pad1->DrawFrame(1,   0.99,  maxdays,   1.01);// all 
      ...
      if(Method == 6) pad1->DrawFrame(1, 900.,  maxdays,   1050.0);  // 6-<A>, 
      if(Method == 7) pad1->DrawFrame(1,   3.0,  maxdays,      5.0);  // 7-<TSn>, 
      if(Method == 8) pad1->DrawFrame(1,   2.5,  maxdays,      3.5);  // 8-<TSx>, 
      if(Method == 9) pad1->DrawFrame(1,   1.5,  maxdays,      2.0);  // 9-<W>, 
      ...
      if(Method ==36&&YEAR==0) pad1->DrawFrame(1,    0.,  maxdays,     0.1);  //36-CapIdErrors 
      if(Method ==36&&YEAR==1) pad1->DrawFrame(1,    0.,  maxdays,     0.01);  //36-CapIdErrors 
      if(Method ==36&&YEAR==2) pad1->DrawFrame(1,    0.,  maxdays,     0.1);  //36-CapIdErrors 
      ...
      if(Method ==37&&YEAR==1) pad1->DrawFrame(1,    0.,  maxdays,      2.);  //37-NbCapIdErrors 
	...
	//
	// for CapIdErrors:
	if(Method == 36) TH2F *twod1= (TH2F*)hfile->Get("h_mapDepth1Error_HE");
	//
	//h_mapTSmaxCalib_HE
	...
	if(Method == 6) {
	  ...
	  TH1F *Ceff37= (TH1F*)hfile->Get("h_nbadchannels_depth1_HE");
	...
	//
	  ...
	  //
	  //
	  if(Method < 6 || Method == 34 || Method == 36) TH2F *twod0= (TH2F*)hfile->Get("h_mapDepth1_HE");
...
///		  cout<<"YY[k1] = "<<YY[k1]<< " rrcount= "<<rrcount<<" rrr= "<<rrr<<" rr= "<<rr<<endl;
    ...
    TString fdir = "plots/";
    if(YEAR == 0) {
      if(FlagOrdinateValue ==1 && FlagWhatToImpose ==2 ) TString fdir = "2010/plots_HE_normalized_5etaRange/";
      if(FlagOrdinateValue ==1 && FlagWhatToImpose ==1 ) TString fdir = "2010/plots_HE_normalized_3etaRange/";
      if(FlagOrdinateValue ==1 && FlagWhatToImpose ==0 ) TString fdir = "2010/plots_HE_normalized/";
      if(FlagOrdinateValue ==0 && FlagWhatToImpose ==0 ) TString fdir = "2010/plots_HE_absolut/";
    }
//
    if(YEAR == 1) {
      if(FlagOrdinateValue ==1 && FlagWhatToImpose ==2 ) TString fdir = "2011/plots_HE_normalized_5etaRange/";
      if(FlagOrdinateValue ==1 && FlagWhatToImpose ==1 ) TString fdir = "2011/plots_HE_normalized_3etaRange/";
      if(FlagOrdinateValue ==1 && FlagWhatToImpose ==0 ) TString fdir = "2011/plots_HE_normalized/";
      if(FlagOrdinateValue ==0 && FlagWhatToImpose ==0 ) TString fdir = "2011/plots_HE_absolut/";
    }
//
    if(YEAR == 2) {
      if(FlagOrdinateValue ==1 && FlagWhatToImpose ==2 ) TString fdir = "plots_HE_normalized_5etaRange/";
      if(FlagOrdinateValue ==1 && FlagWhatToImpose ==1 ) TString fdir = "plots_HE_normalized_3etaRange/";
      if(FlagOrdinateValue ==1 && FlagWhatToImpose ==0 ) TString fdir = "plots_HE_normalized/";
      if(FlagOrdinateValue ==0 && FlagWhatToImpose ==0 ) TString fdir = "plots_HE_absolut/";
    }
//
    if(YEAR < 0) {
      if(FlagOrdinateValue ==1 && FlagWhatToImpose ==2 ) TString fdir = "2010to2012/plots_HE_normalized_5etaRange/";
      if(FlagOrdinateValue ==1 && FlagWhatToImpose ==1 ) TString fdir = "2010to2012/plots_HE_normalized_3etaRange/";
      if(FlagOrdinateValue ==1 && FlagWhatToImpose ==0 ) TString fdir = "2010to2012/plots_HE_normalized/";
      if(FlagOrdinateValue ==0 && FlagWhatToImpose ==0 ) TString fdir = "2010to2012/plots_HE_absolut/";
    }
//
