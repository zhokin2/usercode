...
const Int_t FlagOrdinateValue= 1;// =0 absolut_values; =1 normilized on 1st LED run      0 1 1   
const Int_t FlagWhatToImpose= 0;// =0 random channels; =1 eta range selected             0 0 1  
const Int_t NR0=99;  // number of imposed dependencies(only FlagWhatToImpose= 0 )
//
//______________________________________________________________________________
//
const Int_t NMethods =37;// total number of Methods (all-together 37 methods)
const Int_t NMethodsStart =6;     // since which method to start ( min=  1)
const Int_t NMethodsToUse =6;// choose Number of methods to use ( max= 37)
//
...
//  36+72=108
  ...
  // 1st loop to define number of Methods
...
// //    if(Method > 5 && Method < 21 ) NR = 1;// only averaged values, one dependence only
    ...
    //---=[ SetCanvasDef           ]=----------------
    // Create a new canvas.
    c1 = new TCanvas("zamt3_HBtest","Monte Carlo Study of Z scaling",10,40,800,600);
    c1->Range(0,0,25,18);
    c1->SetFillColor(40);
    c1->Clear();
    //      c1->Divide(2,2);
    ...
    //______________________________________________________________________________
    if(Method == 1) t->DrawLatex(0.9,15.5,"Time dependence of full Amplitude averaged over events of each LED run for some random HB channels");
    if(Method == 2) t->DrawLatex(0.9,15.5,"Time dependence of TSn averaged over events of each LED run for some random HB channels");
    if(Method == 3) t->DrawLatex(0.9,15.5,"Time dependence of TSx averaged over events of each LED run for some random HB channels");
    if(Method == 4) t->DrawLatex(0.9,15.5,"Time dependence of RMS averaged over events of each LED run for some random HB channels");
    if(Method == 5) t->DrawLatex(0.9,15.5,"Time dependence of amplitude Ratio averaged over events of each LED run for some random HB channels");
    if(Method == 6) t->DrawLatex(0.9,15.5,"Time dependence of full Amplitude averaged over events of each LED run and over all HB channels");
    if(Method == 7) t->DrawLatex(0.9,15.5,"Time dependence of TSn averaged over events of each LED run and over all HB channels");
    if(Method == 8) t->DrawLatex(0.9,15.5,"Time dependence of TSx averaged over events of each LED run and over all HB channels");
    if(Method == 9) t->DrawLatex(0.9,15.5,"Time dependence of RMS averaged over events of each LED run and over all HB channels");
    if(Method ==10) t->DrawLatex(0.9,15.5,"Time dependence of amplitude Ratio averaged over events of each LED run and over all HB channels");
    if(Method ==11) t->DrawLatex(0.9,15.5,"Time dependence of Pedestal(cap0) averaged over events of each LED run and over all HB channels");
    if(Method ==12) t->DrawLatex(0.9,15.5,"Time dependence of Pedestal(cap1) averaged over events of each LED run and over all HB channels");
    if(Method ==13) t->DrawLatex(0.9,15.5,"Time dependence of Pedestal(cap2) averaged over events of each LED run and over all HB channels");
    if(Method ==14) t->DrawLatex(0.9,15.5,"Time dependence of Pedestal(cap3) averaged over events of each LED run and over all HB channels");
    if(Method ==15) t->DrawLatex(0.9,15.5,"Time dependence of PedestalWidth(cap0) averaged over events of each LED run and over all HB channels");
    if(Method ==16) t->DrawLatex(0.9,15.5,"Time dependence of PedestalWidth(cap1) averaged over events of each LED run and over all HB channels");
    if(Method ==17) t->DrawLatex(0.9,15.5,"Time dependence of PedestalWidth(cap2) averaged over events of each LED run and over all HB channels");
    if(Method ==18) t->DrawLatex(0.9,15.5,"Time dependence of PedestalWidth(cap3) averaged over events of each LED run and over all HB channels");
    if(Method == 19) t->DrawLatex(0.9,15.5,"Time dependence of Amplitude -1max+2 averaged over events of each LED run and over all HB channels");
    if(Method == 20) t->DrawLatex(0.9,15.5,"Time dependence of normilized fullAmplitude averaged over events of each LED run and over all HB channels");
    if(Method == 21) t->DrawLatex(0.9,15.5,"Time dependence of A_calib averaged over events of each LED run for some random HB channels");
    if(Method == 22) t->DrawLatex(0.9,15.5,"Time dependence of TSn_calib averaged over events of each LED run for some random HB channels");
    if(Method == 23) t->DrawLatex(0.9,15.5,"Time dependence of TSx_calib averaged over events of each LED run for some random HB channels");
    if(Method == 24) t->DrawLatex(0.9,15.5,"Time dependence of W_calib averaged over events of each LED run for some random HB channels");
    if(Method == 25) t->DrawLatex(0.9,15.5,"Time dependence of R_calib averaged over events of each LED run for some random HB channels");
    if(Method == 26) t->DrawLatex(0.9,15.5,"Time dependence of Pedestal-cap0 averaged over events of each LED run for some random HB channels");
    if(Method == 27) t->DrawLatex(0.9,15.5,"Time dependence of Pedestal-cap1 averaged over events of each LED run for some random HB channels");
    if(Method == 28) t->DrawLatex(0.9,15.5,"Time dependence of Pedestal-cap2 averaged over events of each LED run for some random HB channels");
    if(Method == 29) t->DrawLatex(0.9,15.5,"Time dependence of Pedestal-cap3 averaged over events of each LED run for some random HB channels");
    if(Method == 30) t->DrawLatex(0.9,15.5,"Time dependence of PedestalWidth-cap0 averaged over events of each LED run for some random HB channels");
    if(Method == 31) t->DrawLatex(0.9,15.5,"Time dependence of PedestalWidth-cap1 averaged over events of each LED run for some random HB channels");
    if(Method == 32) t->DrawLatex(0.9,15.5,"Time dependence of PedestalWidth-cap2 averaged over events of each LED run for some random HB channels");
    if(Method == 33) t->DrawLatex(0.9,15.5,"Time dependence of PedestalWidth-cap3 averaged over events of each LED run for some random HB channels");
    if(Method == 34) t->DrawLatex(0.9,15.5,"Time dependence of Amplitude -1max+2 averaged over events of each LED run for some random HB channels");
    if(Method == 35) t->DrawLatex(0.9,15.5,"Time dependence of normilized fullAmplitude averaged over events of each LED run for some random HB channels");
    if(Method == 36) t->DrawLatex(0.9,15.5,"Time dependence of CapId errors averaged over events of each LED run for some random HB channels");
    ...
    //              x1   y1     x2     y2
    //    cout<< " **********************FlagOrdinateValue = " << FlagOrdinateValue       <<endl;
    //    cout<< " **********************Method = " << Method       <<endl;
    //    cout<< " **********************FlagWhatToImpose= " << FlagWhatToImpose       <<endl;
    //    cout<< " **********************YEAR = " << YEAR       <<endl;
    if(FlagOrdinateValue !=0 ) {
      if(Method== 1 || Method== 18  || Method== 34  || Method== 35  ) {
	pad1->DrawFrame(1,   0.8,  maxdays,   1.2);// all 
      }
      else if(Method== 6  ){
	pad1->DrawFrame(1,   0.95,  maxdays,   1.05);// all 
      }
      else if(Method== 36 ){
	if(FlagWhatToImpose == 1 ) {pad1->DrawFrame(1,   0.0,  maxdays,   30.0);}
	else {
	  if(YEAR < 0 ) pad1->DrawFrame(1,   0.0,  maxdays,  50.0);
	...
	if(FlagWhatToImpose == 1 ) {pad1->DrawFrame(1,   0.0,  maxdays,   4.0);}
	else {
	  if(YEAR < 0 ) pad1->DrawFrame(1,   0.0,  maxdays,  4.0);
	  if(YEAR == 0 ) pad1->DrawFrame(1,   0.0,  maxdays,  2.0);
	  if(YEAR == 1 ) pad1->DrawFrame(1,   0.0,  maxdays,  2.0);
	  if(YEAR == 2 ) pad1->DrawFrame(1,   0.0,  maxdays,   4.0);
	  //	  if(YEAR == 2 ) cout<< " OK YEAR == 2 FlagWhatToImpose=0 Method == 37" <<endl; 
	}
      }
      else if(Method== 19  ){
	pad1->DrawFrame(1,   0.95,  maxdays,   1.05);// all 
	...
	pad1->DrawFrame(1,   0.90,  maxdays,   1.10);// all 
      ...
      else if( (Method> 20 && Method<26)   ){
	pad1->DrawFrame(1,   0.85,  maxdays,   1.15);// all 
      }
      else if( (Method>25 &&Method< 30 ) && YEAR < 0  ){
	pad1->DrawFrame(1,   0.98,  maxdays,   1.02);// all 
      }
      else {
	pad1->DrawFrame(1,   0.99,  maxdays,   1.01);// all 
      ...
      if(Method == 6) pad1->DrawFrame(1, 950.,  maxdays,   1050.0);  // 6-<A>, 
      if(Method == 7) pad1->DrawFrame(1,   3.8,  maxdays,      3.9);  // 7-<TSn>, 
      if(Method == 8) pad1->DrawFrame(1,   2.5,  maxdays,      3.5);  // 8-<TSx>, 
      if(Method == 9) pad1->DrawFrame(1,   1.5,  maxdays,      1.7);  // 9-<W>, 
	...
	//
	// for CapIdErrors:
	if(Method == 36) TH2F *twod1= (TH2F*)hfile->Get("h_mapDepth1Error_HB");
	//
	...
	//
	//  - P0
	if(Method == 26) TH2F *twod1= (TH2F*)hfile->Get("h_mapDepth1Ped0_HB");
	//  - P1
	if(Method == 27) TH2F *twod1= (TH2F*)hfile->Get("h_mapDepth1Ped1_HB");
	//  - P2
	if(Method == 28) TH2F *twod1= (TH2F*)hfile->Get("h_mapDepth1Ped2_HB");
	//  - P3
	if(Method == 29) TH2F *twod1= (TH2F*)hfile->Get("h_mapDepth1Ped3_HB");
	//
	//    Pedestal Widths :
	//
	//  - PW0
	if(Method == 30) TH2F *twod1= (TH2F*)hfile->Get("h_mapDepth1Pedw0_HB");
	//  - PW1
	if(Method == 31) TH2F *twod1= (TH2F*)hfile->Get("h_mapDepth1Pedw1_HB");
	//  - PW2
	if(Method == 32) TH2F *twod1= (TH2F*)hfile->Get("h_mapDepth1Pedw2_HB");
	//  - PW3
	if(Method == 33) TH2F *twod1= (TH2F*)hfile->Get("h_mapDepth1Pedw3_HB");
	//
	//    Amplitude: -1 max +2:
	//
	//  - A12
	if(Method == 34 || Method == 19) TH2F *twod1= (TH2F*)hfile->Get("h_mapDepth1ADCAmpl12_HB");
	//
	// Ratio of normal channel fullAmplitude to fullAmplitude of corresponding calibration channel
	//
	//  - AtoAc
	if(Method == 35 || Method == 20) TH2F *twod1= (TH2F*)hfile->Get("h_mapGetRMSOverNormalizedSignal_HB");
	//
	//
	//
	...
	if(Method == 6) {
	  ...
	  TH1F *Ceff37= (TH1F*)hfile->Get("h_nbadchannels_depth1_HB");
	...
	//
	  ...
	  //
	  //rest:1-5, (19,20), 21-36
	  //
	  if(Method < 6 || Method == 34 || Method == 36) TH2F *twod0= (TH2F*)hfile->Get("h_mapDepth1_HB");
	  if(Method > 20 && Method < 34) TH2F *twod0= (TH2F*)hfile->Get("h_map_HB");
	  if(Method == 19) TH2F *twod0= (TH2F*)hfile->Get("h_map_HB");
	  if(Method == 35 || Method == 20) TH2F *twod0= (TH2F*)hfile->Get("h_mapGetRMSOverNormalizedSignal0_HB");
	  //
	  // Divide:
	  //
	  ...
	  // k1 are time points(LED runs)
	  // r1 - number of imposed dependencies
	  //
	  ...
	  if(Method != 19 && Method != 20) {
		...
		//		if(YY[k1]< 400.) cout<<"Point:k1="<<k1<<" i="<<irandom<<" j="<<jrandom<<" YY="<<YY[k1]<<endl;
	      }
	      else {                      
// define irandom,jrandom of channel, only ONE TIME, ONCE, at first (RUN) absciss point k1=0:
		//	       if(Method == 1 && r1 == 0) {
		Int_t rr=0;
		Int_t rrcount=0;
		Int_t nlimit=1000;
...
//	Ceff1D->Close();
	...
	for(int k2 = 0; k2<NP; k2++) {
	  if(Method == 1 && r1==0 && (YY[k2]<1060. || YY[k2]> 1190.) ) cout<<"Point:k2="<<k2<<" Method="<<Method<<" YY[k2]="<<YY[k2]<<endl;
	  if(Method == 1 && r1==1 && (YY[k2]<1400. || YY[k2]> 1500.) ) cout<<"Point:k2="<<k2<<" Method="<<Method<<" YY[k2]="<<YY[k2]<<endl;
	}
	...
	for(int k2 = 0; k2<NP; k2++) {
	  ZZ[k2] = YY[k2]/YY[kkk0];// normalization on 1st non zero time point
	  cout<<"Point:k2="<<k2<<" YY[k2]="<<YY[k2]<<" YY[kkk0]="<<YY[kkk0]<<" ZZ[k2]="<<ZZ[k2]<<endl;
	  if(Method == 1 && (ZZ[k2]<0.95 || ZZ[k2]> 1.08) ) cout<<"Point:k2="<<k2<<" Method="<<Method<<" ZZ[k2]="<<ZZ[k2]<<endl;
	  if(Method == 2 && (ZZ[k2]<0.995 || ZZ[k2]> 1.008) ) cout<<"Point:k2="<<k2<<" Method="<<Method<<" ZZ[k2]="<<ZZ[k2]<<endl;
	  if(Method == 4 && (ZZ[k2]<0.99 || ZZ[k2]> 1.01) ) cout<<"Point:k2="<<k2<<" Method="<<Method<<" ZZ[k2]="<<ZZ[k2]<<endl;
	  if(Method == 5 && (ZZ[k2]<0.99 || ZZ[k2]> 1.01) ) cout<<"Point:k2="<<k2<<" Method="<<Method<<" ZZ[k2]="<<ZZ[k2]<<endl;
	}//
	...
	if(FlagWhatToImpose == 1 ) {
	  if(FlagOrdinateValue !=0 ) {
	    ...
	    int kkk0=k2;
	    for(int k2 = 0; k2<NP; k2++) {
	      ZZ[k2] = YY2[k2]/YY2[kkk0];// normalization on 1st non zero time point
	  ...
	  if(FlagOrdinateValue !=0 ) {
	    ...
	    int kkk0=k2;
	    for(int k2 = 0; k2<NP; k2++) {
	      ZZ[k2] = YY3[k2]/YY3[kkk0];// normalization on 1st non zero time point
...
//    TString fdir = "plots/";
    ...
    TString fdir = "plots_HBtest/";
    if(YEAR == 0) {
      if(FlagOrdinateValue ==1 && FlagWhatToImpose ==2 ) TString fdir = "2010/plots_HB_normalized_5etaRange/";
      if(FlagOrdinateValue ==1 && FlagWhatToImpose ==1 ) TString fdir = "2010/plots_HB_normalized_3etaRange/";
      if(FlagOrdinateValue ==1 && FlagWhatToImpose ==0 ) TString fdir = "2010/plots_HB_normalized/";
      if(FlagOrdinateValue ==0 && FlagWhatToImpose ==0 ) TString fdir = "2010/plots_HB_absolut/";
    }
//
    if(YEAR == 1) {
      if(FlagOrdinateValue ==1 && FlagWhatToImpose ==2 ) TString fdir = "2011/plots_HB_normalized_5etaRange/";
      if(FlagOrdinateValue ==1 && FlagWhatToImpose ==1 ) TString fdir = "2011/plots_HB_normalized_3etaRange/";
      if(FlagOrdinateValue ==1 && FlagWhatToImpose ==0 ) TString fdir = "2011/plots_HB_normalized/";
      if(FlagOrdinateValue ==0 && FlagWhatToImpose ==0 ) TString fdir = "2011/plots_HB_absolut/";
    }
//
    if(YEAR == 2) {
      if(FlagOrdinateValue ==1 && FlagWhatToImpose ==2 ) TString fdir = "plots_HB_normalized_5etaRange/";
      if(FlagOrdinateValue ==1 && FlagWhatToImpose ==1 ) TString fdir = "plots_HB_normalized_3etaRange/";
      if(FlagOrdinateValue ==1 && FlagWhatToImpose ==0 ) TString fdir = "plots_HB_normalized/";
      if(FlagOrdinateValue ==0 && FlagWhatToImpose ==0 ) TString fdir = "plots_HB_absolut/";
    }
//
    if(YEAR < 0) {
      if(FlagOrdinateValue ==1 && FlagWhatToImpose ==2 ) TString fdir = "2010to2012/plots_HB_normalized_5etaRange/";
      if(FlagOrdinateValue ==1 && FlagWhatToImpose ==1 ) TString fdir = "2010to2012/plots_HB_normalized_3etaRange/";
      if(FlagOrdinateValue ==1 && FlagWhatToImpose ==0 ) TString fdir = "2010to2012/plots_HB_normalized/";
      if(FlagOrdinateValue ==0 && FlagWhatToImpose ==0 ) TString fdir = "2010to2012/plots_HB_absolut/";
    }
//
//
