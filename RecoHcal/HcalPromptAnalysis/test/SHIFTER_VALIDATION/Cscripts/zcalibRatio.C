...
//	TFile *hfile1= new TFile("test203056.root", "READ");        
	...
	//    getchar();
	//
          TPostScript psfile ("zcalibRatio.ps", 111);
//
   ...
   c1 = new TCanvas("c1","Hcal4test",200,10,700,900);
    ...
    for (int i=1;i<=nx;i++) {
	  ...
	  if(ccc1 < 0.2 )  Diffe_HB->SetBinContent(i,j,ccc1);
      }
    }
      gPad->SetGridy();
      gPad->SetGridx();
      ...
      Diffe_HB->SetZTitle("<RatioCalib> smaller 0.2 HB \b");
    ...
    for (int i=1;i<=nx;i++) {
	  ...
	  if(ccc1 < 0.1 )  Diffe_HE->SetBinContent(i,j,ccc1);
      }
    }
      gPad->SetGridy();
      gPad->SetGridx();
      ...
      Diffe_HE->SetZTitle("<RatioCalib> smaller 0.1 HE \b");
