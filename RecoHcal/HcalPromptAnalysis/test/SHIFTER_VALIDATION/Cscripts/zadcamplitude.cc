...
//      gPad->SetLogx();
	...
	TH2F* Cefw = (TH2F*)twod1->Clone("diffADCAmpl_Depth1_HB");
       nx = Cefw->GetXaxis()->GetNbins();
       ny = Cefw->GetYaxis()->GetNbins();
      for (int i=1;i<=nx;i++) {
	for (int j=1;j<=ny;j++) {
	  if(Cefw->GetBinContent(i,j) !=0 ) {
	    double ccc1 =  Cefw->GetBinContent(i,j) ;
	...
	TH2F* Reff = (TH2F*)ters1->Clone("h_mapDepth1ADCAmpl_HF");
	Reff->Divide(ters1,ters0, 1, 1, "B");
	Reff->Sumw2();
	//    }
    ...
    ///////////////////////////////////////
      ...
      nx = Reff->GetXaxis()->GetNbins();
      ny = Reff->GetYaxis()->GetNbins();
      for (int i=1;i<=nx;i++) {
	for (int j=1;j<=ny;j++) {
	  if(Reff->GetBinContent(i,j) !=0 ) {
	    double ccc1 =  Reff->GetBinContent(i,j) ;
	...
	TH2F* Beff = (TH2F*)iwjr1->Clone("Beff");
	Beff->Divide(iwjr1,iwjr0, 1, 1, "B");
	Beff->Sumw2();
	//    }
    ...
    ///////////////////////////////////////
      ...
      nx = Beff->GetXaxis()->GetNbins();
      ny = Beff->GetYaxis()->GetNbins();
      for (int i=1;i<=nx;i++) {
	for (int j=1;j<=ny;j++) {
	  if(Beff->GetBinContent(i,j) !=0 ) {
	    double ccc1 =  Beff->GetBinContent(i,j) ;
