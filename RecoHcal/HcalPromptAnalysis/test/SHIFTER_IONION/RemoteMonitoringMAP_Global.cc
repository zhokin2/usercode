...
// For occupancy plots. Produces OccPlots_HFx.png (x=1)

    {
      ...
      int maxbins = 0;
      int nx = occhfm->GetXaxis()->GetNbins();
      for (int i=1;i<=nx;i++) {
	...
	if(ccc1>0.) {maxbins = i; if(i>maxbins) maxbins = i;}
      ...
      cout<<"HF maxbins(=LS)=     "<< maxbins <<endl;
      TH1F* uyhfm = new TH1F("uyhfm","", maxbins, 1., maxbins+1.);
      for (int i=1;i<=nx;i++) {
	double ccc1 =  occhfm->GetBinContent(i);
	...
	if(ccc1>0.) uyhfm->Fill(float(i), ccc1);
      ...
      TH1F* uyhfp = new TH1F("uyhfp","", maxbins, 1., maxbins+1.);
      for (int i=1;i<=nx;i++) {
...
//    std::cout << "************>>>   1" << std::endl;
    ...
    for (int i=1;i<=nx;i++) {  //Eta
       for (int j=1;j<=ny;j++) {	// Phi  
	     ...
	     int k_min[5]={0,1,1,4,1}; // minimum depth for each subdet
	     int k_max[5]={0,2,3,4,2}; // maximum depth for each subdet	
//	     std::cout << "************>>>   2 i = " << i << "j = " << j << "sub = " << sub << std::endl;
	     for (int k=k_min[sub];k<=k_max[sub];k++) {  //Depth 
//	       std::cout << "************>>>   3    k = " << k << std::endl;
//	       std::cout << "************>>>   3  Map_SUB[sub][k]->GetBinContent(i,j)=   " <<Map_SUB[sub][k]->GetBinContent(i,j) << "k = " << k << std::endl;
	       if (Map_SUB[sub][k]->GetBinContent(i,j)!=0) {Map_SUB[sub][k]->SetBinContent(i,j,0.5);
//		 std::cout << "************>>>   4" << std::endl;
